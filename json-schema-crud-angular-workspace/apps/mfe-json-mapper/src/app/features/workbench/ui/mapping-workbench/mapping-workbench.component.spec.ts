...
describe('MappingWorkbenchComponent', () => {
  ...
  it('renders review cards as informative summaries without navigation buttons', () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(normalizeText(element.textContent)).toContain('Resumo rapido do estado atual.');
    expect(element.textContent).not.toContain('Anterior');
    expect(normalizeText(element.textContent)).not.toContain('Proximo');
  });
  ...
  it('renders dark code editor helpers for eval and script modes', () => {
    ...
    expect(normalizeText(element.textContent)).toContain('Expressao eval');
    expect(normalizeText(element.textContent)).toContain('JavaScript');
    expect(normalizeText(element.textContent)).toContain('Python');
    expect(normalizeText(element.textContent)).toContain('Java');
    ...
    expect(normalizeText(element.textContent)).toContain('Linguagem ativa: Java');
    expect(normalizeText(element.textContent)).toContain('Java');
    expect(normalizeText(element.textContent)).toContain('JavaScript');
