...
describe('MapperFacadeService', () => {
  ...
  it('should load event types for the dropdown and keep id as the value', () => {
    ...
    expect(service.eventTypes().map((item) => item.name)).toEqual(['Emissao', 'Endosso']);
    expect(service.eventTypeOptions()[0].id).toBe('11111111-1111-1111-1111-111111111111');
    expect(service.isLoadingEventTypes()).toBeFalse();
  });

  it('should append more event types when loading the next page', () => {
    service.loadEventTypes();

    let request = httpMock.expectOne((req) =>
      req.url == 'http://localhost:8080/api/eventos' &&
      req.params.get('page') == '1' &&
      req.params.get('size') == '100'
    );
    ...
    service.loadMoreEventTypes();

    request = httpMock.expectOne((req) =>
      req.url == 'http://localhost:8080/api/eventos' &&
    ...
    expect(service.eventTypes().map((item) => item.name)).toEqual(['Emissao', 'Endosso']);
    expect(service.eventTypesHasNext()).toBeFalse();
  });

  it('should search event types remotely by name and paginate the same query', () => {
    service.updateEventTypeSearchTerm('sorte');
    service.searchEventTypes();
      ...
      items: [
        {
          ...
          description: 'Evento de campanha',
          active: true,
          createdAt: '2026-03-30T10:00:00Z',
          updatedAt: '2026-03-30T10:00:00Z'
        }
    ...
    expect(service.eventTypes().map((item) => item.name)).toEqual(['Sorteio de Brindes', 'Sorteio Especial']);
  });
  ...
  it('should save only the persisted mapper document', () => {
    ...
    service.updatePersistenceMetadata({
      codigoParceiro: 2002,
      eventoParceiro: '22222222-2222-2222-2222-222222222222',
      dataInicioVigencia: '2026-03-28',
      tipoSchema: 'destino',
      versaoSchema: 'v2',
    ...
    expect(request.request.body.codigo_parceiro).toBe(2002);
    expect(request.request.body.id_evento).toBe('22222222-2222-2222-2222-222222222222');
    expect(request.request.body.data_inicio_vigencia).toBe('2026-03-28');
  ...
  it('should migrate v1 config payloads on load', () => {
    ...
    const request = httpMock.expectOne((req) =>
      ...
      req.params.get('codigo_parceiro') == '1001' &&
      !req.params.has('id_evento') &&
      !req.params.has('data_inicio_vigencia') &&
  ...
  it('should restore persistence metadata from v2 config payloads on load', () => {
    ...
    request.flush({
      ...
      config: {
        ...
        persistenceMetadata: {
          codigoParceiro: 3003,
          eventoParceiro: '33333333-3333-3333-3333-333333333333',
          dataInicioVigencia: '2026-03-29',
          tipoSchema: 'destino',
          versaoSchema: 'v9',
          schemaStorageField: 'schema_destino'
        },
        persistenceDocument: {
          codigo_parceiro: 3003,
          tipo_schema: 'destino',
          versao_schema: 'v9',
          id_evento: '33333333-3333-3333-3333-333333333333',
          data_inicio_vigencia: '2026-03-29',
    ...
    expect(service.persistenceMetadata().codigoParceiro).toBe(3003);
    expect(service.persistenceMetadata().eventoParceiro).toBe('33333333-3333-3333-3333-333333333333');
    expect(service.persistenceMetadata().dataInicioVigencia).toBe('2026-03-29');
  ...
  it('should load v2 configs even when ids, rules and schema URIs are missing', () => {
    ...
    request.flush({
      ...
      config: {
        ...
        persistenceMetadata: {
          codigoParceiro: 4004,
          eventoParceiro: '44444444-4444-4444-4444-444444444444',
        ...
        persistenceDocument: {
          codigo_parceiro: 4004,
          tipo_schema: 'destino',
          versao_schema: 'v3',
          id_evento: '44444444-4444-4444-4444-444444444444',
  ...
  it('should load flat persisted documents returned by the backend', () => {
    ...
    request.flush({
      id: 'flat-1',
      codigo_parceiro: 5005,
      id_evento: '55555555-5555-5555-5555-555555555555',
      data_inicio_vigencia: '2026-04-02',
      tipo_schema: 'destino',
      versao_schema: 'v7',
      schema_origem: {
        type: 'object',
    ...
    expect(service.persistenceMetadata().codigoParceiro).toBe(5005);
    expect(service.persistenceMetadata().dataInicioVigencia).toBe('2026-04-02');
  ...
  it('should create script bindings with fallback defaults and preserve schema uris from persisted documents', () => {
    ...
    const request = httpMock.expectOne((req) => req.url == 'http://localhost:8080/api/configs/latest');
    request.flush({
      codigo_parceiro: 1234,
      tipo_schema: 'destino',
      versao_schema: 'v2',
      id_evento: '66666666-6666-6666-6666-666666666666',
      data_inicio_vigencia: '2026-04-03',
      schema_origem: {
        $schema: 'https://json-schema.org/draft/2020-12/schema',
        type: 'object',
        properties: {}
