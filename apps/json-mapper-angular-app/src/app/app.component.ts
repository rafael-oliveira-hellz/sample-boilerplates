...
import { MappingWorkbenchComponent } from '@app/components/mapping-workbench/mapping-workbench.component';
import { PreviewModalComponent } from '@app/components/preview-modal/preview-modal.component';
import { RuleBuilderDrawerComponent } from '@app/components/rule-builder-drawer/rule-builder-drawer.component';
import { SourceSchemaTreeComponent } from '@app/components/source-schema-tree/source-schema-tree.component';
import { TargetSchemaBuilderComponent } from '@app/components/target-schema-builder/target-schema-builder.component';
import { UI_TEXTS } from './content/ui-texts';
import { MapperFacadeService } from '@app/services/mapper-facade.service';
import { SchemaNodeDraft, TargetBindingMode } from '@models/mapper.models';
...
export class AppComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  readonly texts = UI_TEXTS.app;
  private readonly cdr = inject(ChangeDetectorRef);
  dragPointer: { x: number; y: number } | null = null;
  hoveredTargetNodeId = '';
  overlayReady = false;
  previewModalOpen = false;
  private eventTypeSearchDebounceId: ReturnType<typeof setTimeout> | null = null;
  ...
  ngOnDestroy(): void {
    ...
    if (this.eventTypeSearchDebounceId) {
      clearTimeout(this.eventTypeSearchDebounceId);
      this.eventTypeSearchDebounceId = null;
    }
  }
  ...
  onEventTypeSearchChange(value: string): void {
    this.facade.updateEventTypeSearchTerm(value);
    if (this.eventTypeSearchDebounceId) {
      clearTimeout(this.eventTypeSearchDebounceId);
    }
    this.eventTypeSearchDebounceId = setTimeout(() => {
      this.facade.searchEventTypes();
      this.eventTypeSearchDebounceId = null;
    }, 300);
  }
  ...
  private persistentWorkspaceConnections(): PersistentWorkspaceConnection[] {
    ...
    const connections = sourcePaths
      .map<PersistentWorkspaceConnection | null>((sourcePath, index) => {
      ...
      .filter((item): item is PersistentWorkspaceConnection => item !== null);
