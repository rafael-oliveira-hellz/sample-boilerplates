...
async function openJsonMode(panel: Locator): Promise<void> {
  await panel.getByRole('button', { name: 'JSON' }).waitFor({ state: 'visible', timeout: 60000 });
  await panel.getByRole('button', { name: 'JSON' }).click();
}

async function openBuilderMode(panel: Locator): Promise<void> {
  await panel.getByRole('button', { name: 'Builder' }).waitFor({ state: 'visible', timeout: 60000 });
...
async function waitForWorkspace(page: Page): Promise<void> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await page.goto('/');
      await expect(page.getByRole('heading', { name: 'Mapeador visual de schemas JSON' })).toBeVisible({ timeout: 30000 });
      await expect(page.getByRole('button', { name: 'Salvar na API' })).toBeVisible({ timeout: 30000 });
      await expect(page.getByRole('button', { name: /Carregar/i })).toBeVisible({ timeout: 30000 });
      await expect(page.getByTestId('source-panel')).toBeVisible({ timeout: 30000 });
      await expect(page.getByTestId('mapping-workbench')).toBeVisible({ timeout: 30000 });
      await expect(page.getByTestId('target-panel')).toBeVisible({ timeout: 30000 });
      return;
    } catch (error) {
      lastError = error;
...
test.describe('Mapper app', () => {
  ...
  test('salva e recarrega a ultima configuracao mantendo o destino visual', async ({ page }) => {
    ...
    await expect(page.getByText(/Ultima configuracao carregada/i)).toBeVisible();
  ...
  test('recarrega bindings de concat, defaultSource, eval e script no inspector', async ({ page }) => {
    ...
    await expect(workbench.getByTestId('binding-mode-eval')).toHaveClass(/active/);
    await expect(workbench.getByText('Script detectado')).toBeVisible();
    await expect(workbench.getByText('Expressao eval')).toBeVisible();
    await expect(workbench.getByText(/eval-expression\./i)).toBeVisible();
  });
